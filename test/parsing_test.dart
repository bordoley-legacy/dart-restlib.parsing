...
testRuneMatcher(final RuneMatcher matcher, final Iterable<int> matches, final Iterable<int> noMatch) {
  group("RuneMatcher: $matcher", () {
    test("matches", () =>
...
testParser(final Parser parser, final Iterable<String> matches, final Iterable<String> noMatch, int matchLength) {
  group("Parser: $parser", () {
...
testSingleAsciiRuneMatcher(final RuneMatcher matcher, final int cp) =>
    testRuneMatcher(matcher,
...
void constantTests() {
  ...
  testSingleAsciiRuneMatcher(AMPERSAND, toCp("&"));

  testRuneMatcher(ANY, unicode, []);

  testSingleAsciiRuneMatcher(ASTERISK, toCp("*"));
  ...
  testRuneMatcher(CHAR,
        new Range.closed(1, 127).toSet(IntegerDomain.UINT32),
        concat([new Range.single(0),
                new Range.closed(128, 255),
                new Range.closed(65000, 80000)].map(rangeToSet)));

  testSingleAsciiRuneMatcher(CLOSE_PARENTHESES, toCp(")"));

  testSingleAsciiRuneMatcher(COLON, toCp(":"));

  testSingleAsciiRuneMatcher(COMMA, toCp(","));

  testSingleAsciiRuneMatcher(CR, toCp("\r"));
  ...
  testParser(EOF, [""], [" ", "a", "abc", " abc"], 0);

  testSingleAsciiRuneMatcher(EQUALS, toCp("="));

  testSingleAsciiRuneMatcher(DASH, toCp("-"));

  testSingleAsciiRuneMatcher(DQUOTE, toCp("\""));

  testSingleAsciiRuneMatcher(FORWARD_SLASH, toCp("/"));

  testRuneMatcher(HEXDIG,
  ...
  testSingleAsciiRuneMatcher(HTAB, toCp("\t"));

  testParser(INTEGER, ["1234", "4567", "8901"], [" 1234789", "a1", "A11"], 4);

  testSingleAsciiRuneMatcher(LF, toCp("\n"));

  testRuneMatcher(NONE, [], unicode);

  testRuneMatcher(NUMERIC,
        new Range.closed(toCp("0"), toCp("9")).toSet(IntegerDomain.UINT32),
        concat([new Range.closedOpen(0, toCp("0")),
                new Range.open(toCp("9"), 255),
                new Range.closed(65000, 80000)].map(rangeToSet)));

  testRuneMatcher(OCTET,
        new Range.closed(0, 0xFF).toSet(IntegerDomain.UINT32),
        new Range.open(0xFF, 65536).toSet(IntegerDomain.UINT32));

  testSingleAsciiRuneMatcher(OPEN_PARENTHESES, toCp("("));

  testSingleAsciiRuneMatcher(PERIOD, toCp("."));

  testSingleAsciiRuneMatcher(POUND_SIGN, toCp("#"));

  testSingleAsciiRuneMatcher(QUESTION_MARK, toCp("?"));

  testSingleAsciiRuneMatcher(SEMICOLON, toCp(";"));

  testSingleAsciiRuneMatcher(SP, toCp(" "));

  testRuneMatcher(UNICODE, unicode, new Range.open(0x10FFFF, 0x50FFFF).toSet(IntegerDomain.UINT32));

  testRuneMatcher(VCHAR,
